...
CATALOGUE: dict[str, TestSpec] = {
    ...
    "mixed_effects_lm": TestSpec(
        key="mixed_effects_lm",
        label="Linear mixed-effects model",
        question_type="association",
        requires={"outcome": "numeric", "predictors": "mixed", "cluster": "nominal"},
        n_groups=None,
        paired=True,
        nonparametric=False,
        rationale="Repeated measurements per subject (or other clustering) handled via a random-intercept linear mixed-effects model.",
    ),
    "glm_poisson": TestSpec(
        key="glm_poisson",
        label="GLM (Poisson) — counts",
        question_type="association",
        requires={"outcome": "numeric", "predictors": "mixed"},
        n_groups=None,
        paired=False,
        nonparametric=False,
        rationale="Modelling non-negative integer count outcomes with a log link.",
    ),
    "glm_binomial": TestSpec(
        key="glm_binomial",
        label="GLM (Binomial) — proportions / odds",
        question_type="association",
        requires={"outcome": "event_indicator", "predictors": "mixed"},
        n_groups=None,
        paired=False,
        nonparametric=False,
        rationale="Modelling binary outcomes via a logit-linked GLM; equivalent to logistic regression and produces odds ratios.",
    ),
    "glm_gamma": TestSpec(
        key="glm_gamma",
        label="GLM (Gamma) — strictly positive continuous",
        question_type="association",
        requires={"outcome": "numeric", "predictors": "mixed"},
        n_groups=None,
        paired=False,
        nonparametric=False,
        rationale="Modelling strictly-positive continuous outcomes (e.g. hospital costs, time) with a Gamma GLM and log link.",
    ),
    "gee": TestSpec(
        key="gee",
        ...
        paired=True,
        nonparametric=False,
        rationale="Marginal regression for clustered or longitudinal data with an exchangeable working correlation.",
    ),
    "bootstrap_mean_diff": TestSpec(
        key="bootstrap_mean_diff",
        ...
        paired=False,
        nonparametric=True,
        rationale="Distribution-free 95% confidence interval for the difference in means via 9,999 bootstrap resamples.",
    ),
    "permutation_test": TestSpec(
        ...
        paired=False,
        nonparametric=True,
        rationale="Exchange-of-labels exact p-value for the difference in means; robust when sample sizes are small or distributions are odd.",
    ),
    "tost_equivalence": TestSpec(
        key="tost_equivalence",
        label="TOST — two one-sided tests (equivalence)",
        question_type="group_comparison",
        requires={"outcome": "numeric", "groups": "nominal"},
        n_groups=2,
        paired=False,
        nonparametric=False,
        rationale="Tests whether two groups are equivalent within ±margin (a priori). Both one-sided tests must reject the null at α.",
    ),
    "tost_noninferiority": TestSpec(
        key="tost_noninferiority",
        label="TOST — non-inferiority",
        question_type="group_comparison",
        requires={"outcome": "numeric", "groups": "nominal"},
        n_groups=2,
        paired=False,
